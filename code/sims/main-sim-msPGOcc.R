# main-sim-msPGOcc.R: this script runs a simulation study comparing the 
#                     six candidate models when data are generated from a
#                     multi-species occupancy model with no species interactions
...
# Detection
p.det <- 6
alpha.mean <- c(0, runif(p.det - 1, -1, 1))
tau.sq.alpha <- c(0.2, runif(p.det -1, 0, 2))
...
for (j in 1:n.sims) {
  ...
  dat <- simMsOcc(J.x = J.x, J.y = J.y, n.rep = n.rep, N = N, beta = beta, alpha = alpha,
  	        psi.RE = psi.RE, p.RE = p.RE, sp = FALSE, factor.model = factor.model)
...
save(beta.mean.samples, beta.low.samples, beta.high.samples, 
     ...
     run.time.samples, coords.full, pred.indx, beta.true, psi.true,
     file = paste("results/sim-msPGOcc-", Sys.Date(), ".R", sep = ''))
