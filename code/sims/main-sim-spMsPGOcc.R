# main-sim-spMsPGOcc.R: this script runs a simulation study comparing the 
#                       six candidate models when data are generated from a
#                       spatially-explicit multi-species occupancy model  
#                       with no species interactions. a
...
for (j in 1:n.sims) {
  ...
  dat <- simMsOcc(J.x = J.x, J.y = J.y, n.rep = n.rep, N = N, beta = beta, alpha = alpha,
  	        psi.RE = psi.RE, p.RE = p.RE, sp = sp, factor.model = factor.model, 
                phi = phi, sigma.sq = sigma.sq, cov.model = 'exponential')
...
save(beta.mean.samples, beta.low.samples, beta.high.samples, 
     ...
     file = paste("results/sim-spMsPGOcc-", Sys.Date(), ".R", sep = ''))
