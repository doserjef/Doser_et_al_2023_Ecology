...
tmax.pred.vals <- matrix(NA, nrow = nrow(coords.0.sf), ncol = 12)
for (i in 1:ncol(tmax.vals)) {
  tmax.curr.path <- prism_archive_subset("tmax", "monthly", years = curr.year, mon = i)
  # Get absolute file path
  tmax.curr.abs <- pd_to_file(tmax.curr.path)
  ...
  # Extract precipitation value at the starting location of the route. 
  tmp <- st_extract(tmax.curr, at = coords.proj)
  tmax.pred.vals[, i] <- tmp[[1]]
}

# Save values to hard drive -----------------------------------------------
save(tmax.vals, file = paste("data/climate-data/tmax-", curr.year, ".rda", sep = ''))
save(tmax.pred.vals, file = paste("data/climate-data/tmax-", curr.year, "-pred.rda", sep = ''))
