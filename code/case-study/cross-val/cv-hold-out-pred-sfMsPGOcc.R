# cv-hold-out-pred-sfMsPGOcc.R: this script runs a spatially-explicit joint species distribution model
...
#                               This script only uses 25% of the data locations for use in 
...
out <- sfMsPGOcc(occ.formula = ~ scale(bio1) + scale(bio2) + scale(bio8) + scale(bio12) + 
		 ...
		 n.chains = n.chains, n.omp.threads = 3)
# Save results (and pred.indx) --------------------------------------------
save(out, pred.indx, file = paste("results/bbs-hold-out-sfMsPGOcc-", chain, "-chain-", 
		                  Sys.Date(), ".R", sep = ''))
